  ...
  return (
    <div className="grid grid-cols-12 gap-4 p-4 min-h-screen">
      ...
      <div
        className={
          isPreviewFullscreen
            ? `fixed left-0 top-16 h-[calc(100vh-64px)] w-[320px] ${panelsHoverCount > 0 ? "translate-x-0" : "-translate-x-[80%]"} transition-transform duration-300 p-3 space-y-4 z-40`
      ...
      <div className={isPreviewFullscreen ? "fixed inset-x-0 top-16 bottom-0 z-20" : "col-span-6"}>
        <Card className={(isPreviewFullscreen ? "h-full" : "h-[600px]") + " bg-foreground/10 shadow-none relative"}>
          <Button
            className="absolute right-3 top-3 z-10 bg-content1/50 rounded-full p-2"
            onClick={() => setIsPreviewFullscreen((v) => !v)}
            aria-label={isPreviewFullscreen ? "Salir de fullscreen" : "Fullscreen preview"}
          >
            {isPreviewFullscreen ? <ArrowsInSimple size={18} /> : <ArrowsOutSimple size={18} />}
          </Button>
      ...
      <div
        className={
          isPreviewFullscreen
            ? `fixed right-0 top-32 h-[calc(100vh-64px)] w-[360px] ${panelsHoverCount > 0 ? "translate-x-0" : "translate-x-[80%]"} transition-transform duration-300 p-3 space-y-4 z-40`
            : "col-span-3 space-y-4"
        }
        onMouseEnter={() => isPreviewFullscreen && setPanelsHoverCount((c) => c + 1)}
        onMouseLeave={() => isPreviewFullscreen && setPanelsHoverCount((c) => Math.max(0, c - 1))}
